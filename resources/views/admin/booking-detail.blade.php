...
<div style="background:linear-gradient(135deg,var(--blue-800),var(--blue-700));padding:20px 24px;color:white">
  <div style="max-width:760px;margin:0 auto;display:flex;align-items:center;gap:10px">
    <a href="{{ route('admin.bookings') }}" style="padding:7px 14px;border-radius:8px;border:1.5px solid rgba(255,255,255,0.25);color:rgba(255,255,255,0.85);font-size:13px;font-weight:600;text-decoration:none">‹ Kembali</a>
...
<div style="max-width:760px;margin:0 auto;padding:24px 20px 48px">
  <div style="background:var(--white);border-radius:var(--radius);box-shadow:var(--shadow-lg);padding:32px;border:1px solid var(--blue-100)">
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;flex-wrap:wrap;gap:12px">
      <div>
        <div style="font-size:12px;color:var(--gray-400);font-family:monospace;margin-bottom:6px">{{ $kode }}</div>
        <h2 style="font-family:'Playfair Display',serif;font-size:22px;color:var(--blue-900)">{{ $booking->kegiatan }}</h2>
      </div>
      <span class="badge badge-{{ strtolower($status) }}" style="font-size:13px;padding:6px 14px">{{ $status }}</span>
    </div>
    ...
    @if($status === 'DITOLAK' && $booking->rejection_reason)
    ...
    {{-- Verifikasi hanya saat MENUNGGU --}}
    @if($status === 'MENUNGGU')
      ...
      <div id="rejectPanel" style="display:none;margin-top:16px">
        <form method="POST" action="{{ route('admin.booking.reject', $kode) }}">
          @csrf
    ...
    <div style="margin-top:24px">
      <a href="{{ route('admin.bookings') }}" class="btn-sm btn-sm-outline">← Kembali ke Daftar</a>
    </div>
