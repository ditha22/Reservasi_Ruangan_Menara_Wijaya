...
<div style="background:linear-gradient(135deg,var(--blue-800),var(--blue-600));padding:30px 18px;color:white">
  <div style="max-width:920px;margin:0 auto">
    <div style="display:flex;gap:8px;margin-bottom:10px;font-size:13px;color:rgba(255,255,255,0.55)">
      <a href="{{ route('admin.dashboard') }}" style="color:rgba(255,255,255,0.8);text-decoration:none">Dashboard</a>
...
<div style="max-width:920px;margin:0 auto;padding:18px">
  ...
  <form method="GET" action="{{ route('admin.laporan') }}"
    ...
    <div style="flex:1;min-width:170px;background:white;border:1px solid var(--blue-100);border-radius:14px;padding:12px 12px">
      ...
      <select name="month" style="width:100%;border:0;outline:none;font-weight:700">
        @foreach($months as $num=>$name)
          <option value="{{ $num }}" @selected((int)$month === (int)$num)>{{ $name }}</option>
        @endforeach
      </select>
    </div>
    ...
    <div style="flex:1;min-width:220px;background:white;border:1px solid var(--blue-100);border-radius:14px;padding:12px 12px">
      ...
      <select name="room_id" style="width:100%;border:0;outline:none;font-weight:700">
        <option value="" @selected(empty($room_id))>Semua Ruangan</option>
        @foreach($rooms as $r)
          <option value="{{ $r->id }}" @selected((int)$room_id === (int)$r->id)>{{ $r->nama }}</option>
        @endforeach
      </select>
    </div>
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;gap:10px">
      <div>
        <div style="display:flex;align-items:center;gap:8px;color:rgba(255,255,255,0.85);font-size:12px;font-weight:700">
          ...
          Total Peminjaman ({{ $roomLabel }})
        </div>
        <div style="font-size:30px;font-weight:900;line-height:1;margin-top:10px">{{ $total }}</div>

        @if(!is_null($percent))
          <div style="margin-top:8px;font-size:12px;color:rgba(255,255,255,0.85);font-weight:700">
            {{ $percent >= 0 ? '+' : '' }}{{ $percent }}% vs bulan lalu
          </div>
        @else
          <div style="margin-top:8px;font-size:12px;color:rgba(255,255,255,0.85);font-weight:700">
  ...
  <div style="background:white;border-radius:18px;padding:14px;border:1px solid var(--blue-100);box-shadow:var(--shadow);margin-bottom:14px">
    ...
    @else
      <div style="display:flex;flex-direction:column;gap:10px">
        @foreach($roomUsage as $roomName => $count)
          <div style="display:flex;align-items:center;justify-content:space-between;border:1px solid var(--blue-100);border-radius:14px;padding:12px">
            <div style="display:flex;align-items:center;gap:10px">
              ...
              <div style="font-weight:800;color:var(--blue-900);line-height:1.2">{{ $roomName }}</div>
            </div>
            <div style="font-weight:900;color:var(--blue-700)">{{ $count }}<span style="font-weight:700;color:var(--gray-500);margin-left:6px;font-size:12px">sesi</span></div>
          </div>
        @endforeach
      </div>
    @endif
  ...
  <div style="background:white;border-radius:18px;padding:14px;border:1px solid var(--blue-100);box-shadow:var(--shadow);margin-bottom:14px">
    <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:12px">
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <div>
          ...
          <div style="font-size:12px;color:var(--gray-500);font-weight:700">Menampilkan {{ $rows->count() }} data</div>
        </div>
      </div>
    ...
    <div style="overflow:auto;border-radius:14px;border:1px solid var(--blue-100)">
      <table style="width:100%;border-collapse:collapse;min-width:520px">
        ...
        <tbody>
          @forelse($rows as $r)
            @php $st = $statusStyle($r['status']); @endphp
            <tr style="border-top:1px solid var(--blue-100)">
              <td style="padding:10px 12px;font-weight:800;color:var(--blue-900)">{{ $r['tanggal'] }}</td>
              <td style="padding:10px 12px;font-weight:800;color:var(--blue-900)">{{ $r['room'] }}</td>
              <td style="padding:10px 12px;color:var(--gray-700);font-weight:700">{{ $r['opd'] }}</td>
              <td style="padding:10px 12px">
                <span style="display:inline-flex;align-items:center;justify-content:center;padding:6px 10px;border-radius:999px;border:1px solid {{ $st['bd'] }};background:{{ $st['bg'] }};color:{{ $st['fg'] }};font-weight:900;font-size:11px">
                  {{ $st['label'] }}
                </span>
              </td>
            </tr>
  ...
  <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;justify-content:flex-end;margin-bottom:20px">
    <a href="{{ route('admin.laporan.pdf', ['month'=>$month,'year'=>$year,'room_id'=>$room_id]) }}"
    ...
    <a href="{{ route('admin.laporan.excel', ['month'=>$month,'year'=>$year,'room_id'=>$room_id]) }}"
